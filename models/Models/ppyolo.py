...
class ResSPP(nn.Module):   #res SPP 结构
    ...
    def __init__(self, c1 = 1024 ,c2 = 384,act='swish',k = (5,9,13)):
        ...
        if c2 == 1024:
            c_ = c2//2
        self.conv1 = ConvBNLayer(c1, c_, 1, act=act)  # CBR结构
...
class ConvBNLayer(nn.Module):  #CBS,CBR基础结构
...
class CSPResStage(nn.Module):
    def __init__(self, block_fn, c1, c2, n, stride, infor = "backbone",act='relu', attn='eca'):
        ...
        if stride == 2:
            ...
            self.conv_down = None
        self.conv1 = ConvBNLayer(ch_mid, ch_mid // 2, 1, act=act)  #CBR结构 1x1,BN,RELU
        self.conv2 = ConvBNLayer(ch_mid, ch_mid // 2, 1, act=act)  # CBR结构 1x1,BN,RELU
        self.blocks = nn.Sequential(*[block_fn(ch_mid // 2, ch_mid // 2, act=act, shortcut=True) for i in range(n)]) #n个Res Block
        ...
        if infor == "neck":
            ...
            self.blocks = nn.Sequential(*[block_fn(_c2, _c2, act=act, shortcut=False) for i in range(n)])  # n个Res Block  neck中无shortcut
